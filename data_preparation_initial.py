...
def split_dataset(df, time_col, 
                  train_samples=500_000,
                  buffer_samples=200_000,
                  test_samples=80_000):
    ...
    train_df = df.filter(col('row_num') <= train_samples).drop('row_num')
    test_df = df.filter((col('row_num') > train_samples + buffer_samples) & \
                        (col('row_num') <= train_samples + buffer_samples + test_samples))\
                .drop('row_num')
...
def main():
    ...
    train_df, test_df = split_dataset(df, "click_time")

    train_df.repartition(N_PARTITIONS)\
      .write.mode("overwrite")\
      .parquet("s3a://" + BUCKET_PATH + PARQUET_FILE_NAME_TRAIN)
    
    test_df.repartition(N_PARTITIONS)\
      .write.mode("overwrite")\
      .parquet("s3a://" + BUCKET_PATH + PARQUET_FILE_NAME_TEST)
